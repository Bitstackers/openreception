  ...
  factory CallSummary(DailyReport report) {
    ...
    report.callHistory.forEach((history) {
      if (history.inbound) {
        ...
        if (!obCallsByAgent.containsKey(history.assignee)) {
          obCallsByAgent[history.assignee] = 0;
        }
        ...
        obCallsByAgent[history.assignee] = obCallsByAgent[history.assignee] + 1;
      }
    });

    return new CallSummary._internal(
        inboundCount, outboundCount, callsUnAnswered);
  }

  CallSummary._internal(
      this.inboundCount, this.outBoundCount, this.callsUnAnswered);
