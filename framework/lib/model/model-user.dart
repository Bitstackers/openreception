...
abstract class UserDatabase {
  ...
  static Future<Map> getUser(String userEmail, {Connection fromDatabase}) {
    String sql = '''
  ...
  static Future<Map> getUserFromId(int userId, {Connection fromDatabase}) {
      String sql = '''
    SELECT id, name, extension, 
     (SELECT array_to_json(array_agg(name)) 
      FROM user_groups JOIN groups ON user_groups.group_id = groups.id
  ...
  static Future<List<Map>> userList(Connection fromDatabase) {
    const String sql =
'''${SQLMacros}
SELECT
  users.id, 
