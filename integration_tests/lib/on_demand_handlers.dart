...
class Receptionist {
  ...
  Future<shelf.Response> aquire(shelf.Request request) {
    ...
    try {
      ...
      return new Future.value(
          new shelf.Response.notFound('No available receptionists. '
  ...
  Future<shelf.Response> release(shelf.Request request) {
    ...
    try {
      r = _receptionistPool.busy.firstWhere(
          (test_fw.Receptionist rc) => rc.hashCode == receptionistHandle);
      _receptionistPool.release(r);
    } on test_fw.NotAquired {
      return new Future.value(
          new shelf.Response(400, body: 'Not previously aquired'));
    } catch (error, stackTrace) {
      log.severe(error, stackTrace);
      return new Future.value(
          new shelf.Response.internalServerError(body: error.toString()));
    ...
    return r
        .teardown()
        .then((_) => new shelf.Response.ok(JSON.encode({'status': 'ok'})));
  }
  ...
  shelf.Response get(shelf.Request request) {
    ...
    try {
      r = _receptionistPool.elements.firstWhere(
          (test_fw.Receptionist rc) => rc.hashCode == receptionistHandle);
    } catch (error, stackTrace) {
      log.severe(error, stackTrace);
...
class Customer {
  ...
  Future<shelf.Response> aquire(shelf.Request request) {
    ...
    try {
      ...
      return new Future.value(
          new shelf.Response.notFound('No available customers. '
              '${_availableCount} available of ${_totalCount}'));
    } catch (error, stackTrace) {
      log.severe(error, stackTrace);
      return new Future.value(
  ...
  Future<shelf.Response> release(shelf.Request request) {
    ...
    try {
      c = _customerPool.busy
          .firstWhere((test_fw.Customer cu) => cu.hashCode == customerHandle);
  ...
  Future<shelf.Response> hangupAll(shelf.Request request) {
    ...
    try {
      c = _customerPool.busy.firstWhere(
          (test_fw.Customer cu) => cu.hashCode == receptionistHandle);
    } catch (error, stackTrace) {
      log.severe(error, stackTrace);
      return new Future.value(
          new shelf.Response.internalServerError(body: error.toString()));
